...
def outputExcel():
    ...
    sheet.write(0, 4, 'MR总数', style)
    sheet.write(0, 5, 'UR总数', style)
    sheet.write(0, 6, 'SR总数', style)
    sheet.write(0, 7, 'R总数', style)
    sheet.write(0, 8, 'N总数', style)
    sheet.write(0, 9, '水王之王', style)  # 统计抽到whir卡的数量
    sheet.write(0, 10, '欧皇系数', style)
    sheet.write(0, 11, '金币排名', style)
    sheet.write(0, 12, '积分排名', style)
    sheet.write(0, 13, '欧洲排名', style)
    for i in range(len(user_card_list)):
        ...
        sheet.write(i+1, 4, eachCardNum(user_card_list[i]['card'], 0), style)
        sheet.write(i+1, 5, eachCardNum(user_card_list[i]['card'], 1), style)
        sheet.write(i+1, 6, eachCardNum(user_card_list[i]['card'], 2), style)
        sheet.write(i+1, 7, eachCardNum(user_card_list[i]['card'], 3), style)
